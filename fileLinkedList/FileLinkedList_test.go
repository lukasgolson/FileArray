...
func TestFileLinkedList_Add(t *testing.T) {
	...
	list, _ := NewFileLinkedList[util.Number](tmpFile.Name(), false)

	err = list.Add(0, *new(util.Number))
	if err != nil {
	...
	err = list.Add(1, *new(util.Number))
	if err != nil {
		t.Errorf("Add() failed: %v", err)
	}
...
func TestFileLinkedList_AddNonContiguous(t *testing.T) {
	...
	err = list.Add(10, *new(util.Number))
	if err != nil {
		t.Errorf("Add() failed: %v", err)
	}
...
func TestFileLinkedList_GetOne(t *testing.T) {
	...
	// Create a new FileLinkedList and perform the Add operation.
	list, err := NewFileLinkedList[util.Number](tmpFile.Name(), false)
	if err != nil {
		t.Errorf("NewFileLinkedList() failed: %v", err)
	}

	testVal := util.NewNumber(24)
...
func TestFileLinkedList_GetMultiple(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		err := list.Add(0, util.NewNumber(int64(i)))
		if err != nil {
	...
	for i := 0; i < 10; i++ {
		...
		expected := util.NewNumber(int64(i))
		if item != expected {
...
func TestFileLinkedList_Contains(t *testing.T) {
	...
	contains, err = list.Contains(0, util.NewNumber(100))
	if err != nil {
		t.Errorf("Contains() failed: %v", err)
	}
