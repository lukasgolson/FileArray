...
func TestNewFileArray(t *testing.T) {
	...
	fA, err := NewFileArray[util.Number](tmpFile.Name(), false)
	if err != nil {
...
func TestOpenAndInitializeFile(t *testing.T) {
	...
	file, err := openAndInitializeFile[util.Number](tempFile.Name())
...
func TestFileArrayCountEmptyMemoryMap(t *testing.T) {
	...
	fileArray, err := NewFileArray[util.Number](tmpFile.Name(), false)
...
func TestFileArray_FileRetrieve(t *testing.T) {
	...
	for i := 0; i <= 24; i++ {
		_, err = fA.Append(util.NewNumber(int64(i)))
	}
	...
	fA2, err := NewFileArray[util.Number](tmpFile.Name(), false)
